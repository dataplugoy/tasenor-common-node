 ...
 */
class ReportPlugin extends BackendPlugin_1.BackendPlugin {
    constructor(...formats) {
        super();
    ...
    getReportStructure(id) {
        ...
        if (fs_1.default.existsSync(path)) {
            return fs_1.default.readFileSync(path).toString('utf-8');
        }
    }
    /**
    ...
    async constructSqlQuery(db, options, settings) {
        ...
        let sqlQuery = db.select('document.period_id AS periodId', 'document.number AS documentId', 'document.date', 'account.name', 'account.type', 'account.number', db.raw(`CAST(ROUND(${negateSql} * entry.amount * 100) AS BIGINT) AS amount`), 'entry.description')
            .from('entry')
    ...
    async renderReport(db, id, options = {}) {
        ...
        if (options.csv) {
            return (0, __1.data2csv)(report, options);
        }
        return report;
    }
    /**
    ...
    doFiltering(id, entries, options, settings) {
        ...
        if (options.quarter1) {
            filter = (entry) => (0, dayjs_1.default)(entry.date).quarter() <= 1;
        }
        if (options.quarter2) {
            filter = (entry) => (0, dayjs_1.default)(entry.date).quarter() <= 2;
        }
        if (options.quarter3) {
            filter = (entry) => (0, dayjs_1.default)(entry.date).quarter() <= 3;
        }
...
//# sourceMappingURL=ReportPlugin.js.map